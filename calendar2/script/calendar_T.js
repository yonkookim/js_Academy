...
insertSchedule.regist.addEventListener("click",function(){ //문법 & document.forms.insertSchedule의 줌말 ~!!!!
  console.log(this); //이벤트 잘 먹는지 확인
  let y=insertSchedule.year.value;
  let m=insertSchedule.month.value;
  let d=insertSchedule.date.value;
  // let t=insertSchedule.time.value; //9:10 ==>9*60 + 10
  let a_t=insertSchedule.time.value.split(":");
  let t=Number(a_t[0])*60+Number(a_t[1]);
  let val=insertSchedule.text.value;
  (d_09[d])?d_09[d][t]=val:d_09[d]={[t]:val};  //기존 day에 오브젝트가 있는 경우에만 time이 등록됨
  console.log(d_09);
  printCalendar(m,y);
  printMemo(m,y,d);
})
...
insertSchedule.date.value=date;
